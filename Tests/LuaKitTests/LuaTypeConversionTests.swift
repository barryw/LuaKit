...
final class LuaTypeConversionTests: XCTestCase {
    ...
    func testDateLuaConvertible() {
        let date = Date(timeIntervalSince1970: 1640429400) // 2021-12-25 10:30:00 UTC
    ...
    func testOptionalLuaConvertibleSome() {
        ...
        // Push to Lua
        Optional<Int>.push(optional, to: L)
        
        XCTAssertEqual(lua_tointeger(L, -1), 42)
        
        // Pull from Lua
        let pulled = Optional<Int>.pull(from: L, at: -1)
        XCTAssertNotNil(pulled) // This is Optional<Optional<Int>>
        XCTAssertEqual(pulled ?? nil, 42)
    ...
    func testDictionaryLuaConvertible() {
        ...
        // Push to Lua
        Dictionary<String, Int>.push(dict, to: L)
        
        XCTAssertEqual(lua_type(L, -1), LUA_TTABLE)
        
        // Pull from Lua
        let pulled = Dictionary<String, Int>.pull(from: L, at: -1)
        XCTAssertNotNil(pulled)
    ...
    func testComplexDictionaryConversion() {
        ...
        // Push to Lua
        Dictionary<String, String>.push(dict, to: L)
        
        // Pull and verify
        let pulled = Dictionary<String, String>.pull(from: L, at: -1)
        XCTAssertNotNil(pulled)
        XCTAssertEqual(pulled?.count, 3)
    ...
    func testOptionalProtocol() {
        ...
        let nil_optional: Int? = nil
        let nil_wrapped = nil_optional.optional
        XCTAssertNil(nil_wrapped)
    }
}